...
const PDF_RED_COLOR: [number, number, number] = [239, 68, 68];
...
const drawHeader = ({
  ...
  doc.setFontSize(9);
  doc.text(
    formatDateTime24({ value: new Date(), locale: "de-DE" }),
    pageWidth - 40,
    58,
    {
      align: "right",
...
const drawOrderReportHeader = (
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(22);
  doc.text("DeliveryWays", 40, 34);
  doc.setFontSize(13);
  doc.text(restaurantName || "Restaurant", 40, 56);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  doc.text(
    `${formatReportDate(period?.fromDate)} - ${formatReportDate(period?.toDate)}`,
    pageWidth - 40,
    55,
    { align: "right" },
  );
...
const drawOrderReportFooter = (doc: jsPDF) => {
  const pageCount = doc.getNumberOfPages();
  const pageWidth = doc.internal.pageSize.getWidth();
  const pageHeight = doc.internal.pageSize.getHeight();
  ...
  for (let page = 1; page <= pageCount; page += 1) {
    ...
    doc.setTextColor(...PDF_GRAY_COLOR);
    doc.setFontSize(8);
    doc.text("Generated by DeliveryWays", 40, pageHeight - 18);
...
const renderOrderPeriodReport = ({
  ...
  currency?: string;
  period?: { fromDate: string; toDate: string };
  data: OrderPeriodReportData;
}) => {
  drawOrderReportHeader(doc, restaurantName, period);

  autoTable(doc, {
    startY: 96,
    head: [["Order ID", "Date", "Payment Method", "Payment Status", "Amount"]],
    body: (data.orders ?? []).map((order) => [
      order.id,
      formatReportDate(order.createdAt),
      prettyLabel(order.paymentMethod),
      prettyLabel(order.paymentStatus),
      formatInvoiceCurrency(order.totalAmount, currency),
    ]),
    margin: { top: 90, left: 40, right: 40, bottom: 150 },
    theme: "plain",
    styles: {
      font: "helvetica",
      fontSize: 8,
    ...
    didDrawPage: ({ pageNumber }: HookData) => {
      if (pageNumber > 1) drawOrderReportHeader(doc, restaurantName, period);
    },
  } as any);

  const summaryY = getFinalY(doc, 120) + 24;
  addSectionTitle(doc, "Summary", summaryY);
  autoTable(doc, {
    startY: summaryY + 10,
    body: [
      ["Offline/Cash Orders", String(data.offlineOrderCount ?? 0)],
      [
        "Offline/Cash Total",
        formatInvoiceCurrency(data.offlineAmount ?? 0, currency),
      ],
      ["Online Payment Orders", String(data.onlineOrderCount ?? 0)],
      [
        "Online Payment Total",
        formatInvoiceCurrency(data.onlineAmount ?? 0, currency),
      ],
      ["Total Orders", String(data.totalOrders ?? 0)],
      [
        "Total Revenue",
        formatInvoiceCurrency(data.totalRevenue ?? 0, currency),
      ],
    ],
    ...
    columnStyles: {
      ...
      1: { halign: "right", cellWidth: 265 },
    },
  } as any);

  drawOrderReportFooter(doc);
};
...
export const downloadRestaurantDashboardReportPdf = ({
  ...
  if (reportType === "order") {
    renderOrderPeriodReport({
      doc,
      restaurantName: restaurantName || "Restaurant",
      currency,
      period,
      data: (data ?? {}) as OrderPeriodReportData,
    });
    doc.save(
      `${sanitizeFileName(title || "order-report")}-${new Date()
        .toISOString()
        .slice(0, 10)}.pdf`,
    );
    return;
...
export const downloadRestaurantInvoicePdf = (invoice: AdminInvoice) => {
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      [
        "Delivery Fee",
        formatInvoiceCurrency(invoice.deliveryFee || 0, currency),
      ],
      [
        "Discount",
        `-${formatInvoiceCurrency(invoice.discountAmount || 0, currency)}`,
      ],
      [
        "Wallet Applied",

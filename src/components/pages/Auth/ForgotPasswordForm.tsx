...
const ForgotPasswordForm = () => {
  ...
  const onSubmit = async (values: ForgotPasswordFormValues) => {
    try {
      ...
      window.location.href = `/reset-password?email=${encodeURIComponent(values.email)}&restaurantId=${values.restaurantId || ""}`;
    } catch (error) {
  ...
  return (
    <div className="grid min-h-screen grid-cols-1 bg-white lg:grid-cols-2">
      ...
      <div className="flex items-center justify-center px-6">
        <div className="w-full max-w-[480px] rounded-[24px] border border-gray-200 px-8 py-10">
          ...
          <h1 className="text-center text-[26px] font-semibold text-[#EC5834]">
            Forgot password?
          </h1>
          ...
          <form className="mt-8 space-y-6" noValidate onSubmit={handleSubmit(onSubmit)}>
            ...
            <Button
              type="submit"
              className="h-[52px] w-full rounded-[14px] bg-primary text-base hover:bg-red-800"
